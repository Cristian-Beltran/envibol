      ...
      >
        <div class="rounded-t bg-white mb-0 px-6 py-6">
          <div class="text-center flex justify-between">
            <h6 class="text-blueGray-700 text-xl font-bold">Molinete</h6>
          </div>
        </div>
            ...
            <div class="w-full mx-auto p-4 md:py-8">
              <div class="flex items-center justify-between">
                <div class="text-center flex items-center mb-4">
                  <router-link to="/admin/turnstile" v-slot="{ href, navigate }">
                    <button
                      :href="href"
                      :click="navigate"
...
export default {
        ...
        const request = async () => {
          try {
            if (!this.$route.query.id)
              await createTurnstileRequest(this.formData);
            else
              await updateTurnstileRequest(this.$route.query.id, this.formData);
            this.$router.push("/admin/turnstile");
          } catch (error) {
  ...
  async created() {
    if (this.$route.query.id) {
      const res = await getTurnstileRequest(this.$route.query.id);
